package org.edec.student.journalOfAttendance.manager;
...
public class JournalOfAttendanceManager extends DAO {
    ...
    public GroupSemesterModel getGroupSemester (Long idHum, Long idSem) {
        String query = "SELECT\tDG.groupname, LGS.course, LGS.id_link_group_semester AS idLGS,\n" +
                "\tLGS.dateofbeginsemester, LGS.dateofendsemester\n" + "FROM\tstudentcard SC\n" +
                "\tINNER JOIN student_semester_status SSS USING (id_studentcard) \n" +
                "\tINNER JOIN link_group_semester LGS USING (id_link_group_semester)\n" +
                "\tINNER JOIN semester SEM USING (id_semester)\n" + "\tINNER JOIN dic_group DG USING (id_dic_group)\n" +
                "WHERE\tSC.id_humanface = :idHum AND SEM.id_semester = :idSem AND SSS.is_academicleave = 0 AND SSS.is_deducted = 0";
        Query q = getSession().createSQLQuery(query)
                .addScalar("groupname")
                .addScalar("course")
                .addScalar("idLGS", LongType.INSTANCE)
                ...
                .addScalar("dateOfEndSemester")
                .setResultTransformer(Transformers.aliasToBean(GroupSemesterModel.class));
        q.setLong("idHum", idHum).setLong("idSem", idSem);
        List list = getList(q);
        return list.size() == 0 ? null : (GroupSemesterModel) list.get(0);
    }
    ...
    public List<MonthAttendSubjectsModel> getMonthAttendModel (Long idLGS, Date dateOfBegin, Date dateOfEnd) {
        String query = "SELECT\tCAST(mDay AS DATE) AS day,count(AT.*)\n" + "FROM \tgenerate_series('" +
                       ...
                       "\t\tAND AT.id_link_group_semester_subject IN (SELECT id_link_group_semester_subject FROM link_group_semester_subject WHERE id_link_group_semester = :idLGS) and at.attend <>-1\n" +
                       "GROUP BY mDay\n" + "ORDER BY mDay";
        Query q = getSession().createSQLQuery(query)
                              .addScalar("day")
    ...
    public List<StudentAttendanceModel> getStudentByLGS (Long idLGS) {
        String query = "SELECT HF.family, HF.name, HF.patronymic, SSS.id_student_semester_status AS idSSS\n" + "FROM humanface HF\n" +
                       ...
                       "WHERE SSS.is_deducted = 0 AND SSS.is_academicleave = 0 AND SSS.is_transfer_student = 0\n" +
                        "AND LGS.id_link_group_semester = :idLGS AND LGS.id_dic_group = SC.id_current_dic_group\n" +
    ...
    public List<AttendanceModel> getAttendanceByLGSAndDate (Long idLGS, Date visitDate) {
        String query = "SELECT\n" + "\tAT.id_student_semester_status AS idSSS, AT.id_link_group_semester_subject AS idLGSS,\n" +
                       ...
                       "\tLGSS.id_link_group_semester = :idLGS AND AT.visitdate = '" +
                       DateConverter.convertDateToSQLStringFormat(visitDate) + "'";
        Query q = getSession().createSQLQuery(query)
                              .addScalar("idSSS", LongType.INSTANCE)
    ...
    public List<AttendanceModel> getSubjectAttendanceByLGSAndDate (Long idLGS, Date visitDate) {
        String query = "SELECT\tDISTINCT AT.id_link_group_semester_subject AS idLGSS, DS.subjectname, AT.pos, AT.lesson\n" +
                       "FROM\tattendance AT\n" + "\tINNER JOIN link_group_semester_subject LGSS USING (id_link_group_semester_subject)\n" +
                       "\tINNER JOIN subject S USING (id_subject)\n" + "\tINNER JOIN dic_subject DS USING (id_dic_subject)\n" +
                       "WHERE\tLGSS.id_link_group_semester = :idLGS AND AT.visitdate = '" +
                       DateConverter.convertDateToSQLStringFormat(visitDate) + "'\n" + "ORDER BY pos";
        Query q = getSession().createSQLQuery(query)
                              .addScalar("idLGSS", LongType.INSTANCE)
                              .addScalar("subjectname")
    ...
    public void createAttendance (AttendanceModel attendance) {
        String query =
                "INSERT INTO attendance (id_link_group_semester_subject, id_student_semester_status, pos, attend, visitdate, lesson)\n" +
                "VALUES (:idLGSS, :idSSS, :pos, :attend, :visitdate, :lesson)";
        Query q = getSession().createSQLQuery(query);
        q.setLong("idLGSS", attendance.getIdLGSS())
         .setLong("idSSS", attendance.getIdSSS())
         .setInteger("pos", attendance.getPos())
         .setInteger("attend", attendance.getAttend())
         .setDate("visitdate", attendance.getVisitDate())
         .setBoolean("lesson", attendance.getLesson());
        executeUpdate(q);
    }
    ...
    public void deleteAttendanceByDate (Date day, Long idLGS) {
        ...
        Query q = getSession().createSQLQuery(query).setDate("selectedDay", day).setLong("idLGS", idLGS);
        executeUpdate(q);
    }
}
