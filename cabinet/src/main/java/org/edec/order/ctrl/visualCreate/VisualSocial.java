...
public class VisualSocial extends VisualAbstract {
    private CreateOrderService service = new CreateOrderServiceESO();

    public VisualSocial(Window winCreateOrder, IndexPageCtrlDelegate delegate, OrderRuleModel rule, Long sem, InstituteModel institute, FormOfStudy formOfStudy) {
        super(winCreateOrder, delegate, rule, sem, institute, formOfStudy);
    }

    @Override
    public void draw(Component component) {
        while (component.getFirstChild() != null) component.removeChild(component.getFirstChild());

        final Vlayout vLayout = new Vlayout();
        final Hbox hbox = new Hbox();
        final Label label1;

        if(rule.getName().equals("Социальный приказ(новые справки)")) {
        ...
        if(rule.getName().equals("Социальный приказ(новые справки)")) vLayout.appendChild(inSession);
            ...
            public void onEvent(Event event) throws Exception {
                ...
                try {
                    ...
                    if (id != null) {
                        ...
                        String pathCreatedFile = service.createOrderByParams(institute, FileModel.SubTypeDocument.SOCIAL, sem, String.valueOf(id), null);
